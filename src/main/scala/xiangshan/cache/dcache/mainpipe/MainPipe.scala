...
//  val amo_bitmask = FillInterleaved(8, s3_req.amo_mask)
  ...
  // TODO:555 dataWritePort or metaWritePort
  ...
  io.tag_write.valid := ((s3_fire_dup_for_meta_w_valid  && update_meta_dup_for_meta_w_valid) || s3_fire_dup_for_tag_w_valid) && s3_req_miss_dup_for_tag_w_valid
  io.tag_write.bits.idx := s3_idx_dup(4)
  io.tag_write.bits.way_en := s3_way_en_dup(2)

  // need to cat coh and tag
  io.tag_write.bits.tag := Cat(new_coh.asUInt, get_tag(s3_req_addr_dup(4)))
  io.tag_write.bits.vaddr := s3_req_vaddr_dup_for_data_write

  io.tag_write_intend := (s3_req_miss_dup(7) || update_meta_dup_for_meta_w_valid ) && s3_valid_dup(11)
