...
class MainPipe(implicit p: Parameters) extends DCacheModule with HasPerfEvents with HasL1PrefetchSourceParameter {
  ...
  XSPerfAccumulate("store_has_invalid_way_but_select_valid_way", io.replace_way.set.valid && wayMap(w => !meta_resp(w).asTypeOf(new ClientMetadata).isValid()).asUInt.orR && s1_need_replacement && s1_repl_coh.isValid())
...
//  val amo_bitmask = FillInterleaved(8, s3_req.amo_mask)
  ...
  io.status.s1.valid := s1_valid
  io.status.s1.bits.set := RegEnable(get_idx(s0_req.vaddr), s0_fire)
  io.status.s1.bits.way_en := s1_way_en
  io.status.s2.valid := s2_valid && !RegEnable(s1_req.replace, s1_fire)
  io.status.s2.bits.set := RegEnable(get_idx(s1_req.vaddr), s1_fire)
  io.status.s2.bits.way_en := RegEnable(s1_way_en, s1_fire)
  io.status.s3.valid := s3_valid && !RegEnable(s2_req.replace, s2_fire_to_s3)
  io.status.s3.bits.set := RegEnable(get_idx(s2_req.vaddr), s2_fire_to_s3)
  io.status.s3.bits.way_en := RegEnable(s2_way_en, s2_fire_to_s3)
  dontTouch(io.status)
