...
/** S1:
  ...
  */
  ...
  val s1_meta_resp = io.tag_resp.map{tag => ClientMetadata(tag(tagBits + ClientStates.width - 1, tagBits))}
  ...
  val s1_hit_meta = Mux(s1_tag_match.orR, Mux1H(s1_tag_match, wayMap((wayid: Int) => {s1_meta_resp(wayid)})), s1_fake_meta.coh)
